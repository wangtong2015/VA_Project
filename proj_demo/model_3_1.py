...
class VAMetric(nn.Module):
    def __init__(self):
        ...
        self.vfeat_fc_3 = nn.Conv2d(1, 32, kernel_size=(64,1))
    ...
    def forward(self, vfeat, afeat):
        ...
        vfeat = F.relu(vfeat, inplace= True)
        vfeat = self.vfeat_fc_3(vfeat)                                   #batch * 32* 1 * 120
        vfeat = torch.transpose(vfeat, 1, 2)                             #batch * 1 * 32 * 120
        vfeat = F.relu(vfeat, inplace= True)
        vfeat = vfeat.contiguous()
        vfeat = vfeat.view(-1, 3840)
