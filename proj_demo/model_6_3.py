...
class VAMetric(nn.Module):
    def __init__(self):
        ...
        self.VFeatPool = FeatAggregate(1024, 256)  #batch* 120 *128
        self.AFeatPool = FeatAggregate(128, 128)   #batch* 120 *128
        #merge batch* 3 * 120 * 128
        self.conv_1 = nn.Conv2d(3, 60, kernel_size=(3, 128), padding=(1,0), stride=(2,1)) #batch*64*60*1
        # transpose batch*1*60*64
        self.mpool_1 = nn.MaxPool2d(kernel_size=(3,3), stride=(2,2), padding=(1,1)) # batch*1*30*30
        self.BN_1 = nn.BatchNorm2d(1)
        self.conv_2 = nn.Conv2d(1,48, kernel_size=(3,3),padding=(1,1),stride=(2,1)) # batch*48*15*30
        self.BN_2 = nn.BatchNorm2d(48)
        self.mpool_2 = nn.MaxPool2d(kernel_size=(3,2)) #batch*48*5*15
        self.conv_3 = nn.Conv2d(48,96,kernel_size=(5,3),padding=(0,1),stride=(1,2)) # batch*96*1*8
        # transpose batch*1*96*8
        self.BN_3 = nn.BatchNorm2d(1)
        self.fc_1 = nn.Linear(96*8,1)
    ...
    def forward(self, vfeat, afeat):
        ...
        vfeat = vfeat.view(vfeat.size(0), 1, vfeat.size(1), vfeat.size(2))     #batch * 1 * 120*256
        afeat = afeat.contiguous()
        afeat = afeat.view(afeat.size(0), 1, afeat.size(1), afeat.size(2))
        avfeat = torch.cat((afeat, vfeat[:,:,:,0:128], vfeat[:,:,:,128:256]), 1)
